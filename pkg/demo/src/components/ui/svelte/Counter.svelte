...
<div class="rounded-2xl border border-slate-200/60 bg-white/70 p-6 flex flex-col gap-4 shadow-sm">
  <div class="flex items-center justify-between border-b border-slate-200/60 pb-3">
    <span class="text-xs uppercase tracking-wider text-slate-500 font-bold">Svelte Island</span>
    <span class="rounded-full bg-slate-100 border border-slate-200/60 px-2.5 py-0.5 text-xs font-semibold text-slate-600">client:load</span>
  </div>
  ...
  <div class="mt-2 border-t border-slate-200/60 pt-3">
    <span class="text-xs text-slate-400 block mb-2 font-semibold uppercase tracking-wider">{$t('frameworks:changeLanguage.svelte')}</span>
